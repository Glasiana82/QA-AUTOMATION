...
test('deve fazer login e cadastrar um filme com sucesso', async ({ page }) => {
    ...
    const filmeLinha = page.locator(`#${filmeId}`);

    // ===== ALUGAR O FILME =====
    // ACEITA O ALERTA ANTES DO CLIQUE
    page.once('dialog', async dialog => {
    console.log(`Dialog message: ${dialog.message()}`);

    // deixa a mensagem visível por 4 segundos
    ...
    await filmeLinha.getByRole('button', { name: 'ALUGAR' }).click();


    await expect(filmeLinha).toContainText('Alugado');


    console.log(`✅ Filme "${filmeTitle}" foi alugado com sucesso!`);
    await page.waitForTimeout(4000);
